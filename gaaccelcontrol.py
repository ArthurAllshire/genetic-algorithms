...
def fitness(individual, print_dist=False):
    ...
    for dist in range(3):
        ...
        distance = 0
        since_target = 0
        target = smallest_dist * 10**(dist)
        time_until_settled = 0
        correct = True
        while since_target < 2 and time_until_settled < 100:
            if print_dist:
                print("TIME PERIODS: ", time_until_settled, " DISTANCE: ", distance, " TARGET: ", target)
            output = evaluate_individual(chrom, velocity, distance, target, print_dist)
            if output >= 10:
                output = 10
            elif output <= -10:
                output = -10
            ...
            if time_until_settled > 10 and distance == 0 or distance > target * 10 or distance < 0:
                break
        times_until_settled.append(time_until_settled)
        value = 0
        if since_target == 0 and not (abs(distance - target) <= 2):
            correct = False
            if  0 < distance > target:
                if print_dist:
                    print("TARGET: ", target, " FIRST TRIPPED")
                value = 1/(abs(target - distance)*10000)
            elif 0 < distance < target:
                if print_dist:
                    print("TARGET: ", target, " SECOND TRIPPED")
                value = 1/(abs(target - distance)*1000)
            else:
                if print_dist:
                    print("TARGET: ", target, " THIRD TRIPPED")
                value = 1/10000000000000000
        else:
            if print_dist:
                print("TARGET: ", target, " REACHED TARGET")
